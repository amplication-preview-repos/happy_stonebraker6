/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PlacesWhereUniqueInput } from "./PlacesWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { PlacesUpdateInput } from "./PlacesUpdateInput";

@ArgsType()
class UpdatePlacesArgs {
  @ApiProperty({
    required: true,
    type: () => PlacesWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => PlacesWhereUniqueInput)
  @Field(() => PlacesWhereUniqueInput, { nullable: false })
  where!: PlacesWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => PlacesUpdateInput,
  })
  @ValidateNested()
  @Type(() => PlacesUpdateInput)
  @Field(() => PlacesUpdateInput, { nullable: false })
  data!: PlacesUpdateInput;
}

export { UpdatePlacesArgs as UpdatePlacesArgs };
